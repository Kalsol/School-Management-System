...
// use App\Http\Controllers\SupportTeam\SystemController;
...
Route::group(['middleware' => ['auth','checkForPasswordUpdate',]], function () {
    
    // AJAX Routes
    Route::prefix('ajax')->name('ajax.')->group(function () {
            Route::get('/sections', [AjaxController::class, 'getSectionsByClass'])->name('sections');
            Route::get('/subjects', [AjaxController::class, 'getSubjectsByClass'])->name('subjects');
            Route::get('/results-preview', [AjaxController::class, 'resultsPreview'])->name('results.preview');
    });
    ...
    Route::group(['namespace' => 'SupportTeam',], function(){
        ...
        Route::group(['prefix' => 'attendance'], function () {
            // The page where Admin clicks "Start Camera"
            Route::get('/manage', [AttendanceController::class, 'index'])->name('attendance.index');
                
            // The page to see student history/reports
            Route::get('/student-report', [AttendanceController::class, 'studentReport'])->name('attendance.student_report');
            
            // The page to see teacher history/reports
            Route::get('/teacher-report', [AttendanceController::class, 'teacherReport'])->name('attendance.teacher_report');
            
            // The action that triggers the session (optional: for shell_exec)
            Route::post('/start-student-session', [AttendanceController::class, 'startStudentFaceRecognition'])->name('attendance.start');
                   
            // The action that triggers the session (optional: for shell_exec)
            Route::post('/end-session', [AttendanceController::class, 'endSession'])->name('attendance.end_session');
    ...
    Route::group(['prefix' => 'ajax'], function() {
        ...
        Route::get('get_states/{nationality_id}', 'AjaxController@get_states')->name('get_states');
        Route::get('get_class_sections/{class_id}', 'AjaxController@get_class_sections')->name('get_class_sections');
        Route::get('get_class_subjects/{class_id}', 'AjaxController@get_class_subjects')->name('get_class_subjects');
        // Route::get('assessment-type/check', [AssessmentController::class, 'checkAssessmentType'])->name('ajax.assessment-type.check');
