...
class ExamController extends Controller
{
    ...
    public function store(ExamCreate $req)
    {
        ...
        $find = Exam::where('year', $data['year'])
                ->where('name', $data['name'])
                ->get();
        
        // dd($find);
        if($find->count() > 0){
            return back()->with('flash_danger', __('Cannot create Marking Session with same name at the same year'));
        }
        
        // if ($find->term == $data['term']) {
        //     return back()->with('flash_danger', __('msg.term_already_exists'));
        // }
    ...
    {
        $data = $req->only(['marking_name', 'term']);
